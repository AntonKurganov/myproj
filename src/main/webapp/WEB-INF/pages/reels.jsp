...
<head>
    ...
    <script type="text/javascript">
        ...
        function valid (form) {
            ...
            var bearingNumber = form.bearingNumber.value;
            ...
            }else if (bearingNumber==0 || isEmpty(bearingNumber)){
                fail = "Input the bearing number of the reel";
            } else if (isEmpty(type)){
                fail = "Input the type of the reel";
            }
            if (fail) alert(fail);
            else form.submit();
        }
...
<div class="container-fluid">
    ...
    <div class="col-md-8">
        ...
        <div class="row">
            <c:forEach items="${reels}" var="reel">
                <div class="col-md-4">
                    <div class="thumbnail">
                        <div class="caption">
                            <b style="font-size: 20px; min-height: 56px;" class="pull-left">${reel.name}</b>
                            <a href="/reel_${reel.id}"><img src=${reel.photoLink} width="250px" height="250px" alt=""></a>
                            <b style="font-size: 16px;" class="pull-right">${reel.country}      ${reel.bearings}b      ${reel.price}₴</b>
                            <br>
                            <br>
                            <a href="/reel_${reel.id}" class="btn btn-success pull-left">More details <i class="fa fa-arrow-right"></i></a>
                            <c:if test="${role=='ADMIN'}">
                                <form action="deleteReel" method="post">
                                    <button class="btn btn-danger pull-right" name="id" value="${reel.id}"><i class="fa fa-window-close-o" aria-hidden="true"></i></button>
                                </form>
                            </c:if>
                            <c:if test="${role=='USER'}">
                                <form action="addreelToBasket" method="post">
                                    <button class="btn btn-success pull-right" name="id" value="${reel.id}"><i class="fa fa-shopping-basket" aria-hidden="true"></i></button>
                                </form>
                            </c:if>
                            <br>
                            <br>
        ...
        <c:if test="${role=='ADMIN'}">
            <div class="row" >
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#newReel">Add Reel</button>
            </div>
        </c:if>
    </div>
    <c:if test="${reels.size()!=0}">
        <div class="col-md-2">
            <h3 style="font-family: 'Book Antiqua'">Sort by:</h3>
            <div id="accordion" class="panel-group">
                <br>
                <form action="reelsFromCountry">
                ...
                <br>
                <form action="reelsByBN">
                    <div class="panel panel-success">
                        <div class="panel-heading">
                            <h4 class="panel-title">
                                <a href="#collapse-BN" data-parent="#accordion" data-toggle="collapse" class="spoiler"><i class="fa fa-chevron-right" aria-hidden="true"></i><b> Bearing Number</b></a>
                            </h4>
                        </div>
                        <div id="collapse-BN" class="panel-collapse collapse">
                            <div class="panel-body">
                                <c:forEach items="${bearings}" var="bearingNumber">
                                    <li style="list-style: none">
                                        <input type="radio" value=${bearingNumber} name="bearingNumber"/>${bearingNumber}
                                    </li>
                                </c:forEach>
                            </div>
                            <input type="submit" value="SORT" class="btn-default">
                ...
                <br>
                <form action="reelsByType">
                    <div class="panel panel-success">
                        <div class="panel-heading">
                            <h4 class="panel-title">
                                <a href="#collapse-type" data-parent="#accordion" data-toggle="collapse" class="spoiler"><i class="fa fa-chevron-right" aria-hidden="true"></i><b> Type</b></a>
                            </h4>
                        </div>
...
<br>
<br>
<div class="container" style="background-color: white; opacity: 0.8; width: 100%; padding: 20px; bottom: 0px;<c:if test="${reels.size()<4}">position: absolute;</c:if>">
...
<div class="modal fade" id="newReel">
    <div class="modal-dialog modal-md">
        <div class="modal-content-contacts">
            <div class="modal-header">
                <p class="modal-title" align="center"><font face="joan" size="5" color="darkslategray">Add new Reel</font></p>
            </div>
            <div class="modal-body">
                <form action="newReel" onsubmit="valid(document.getElementById('form')); return false" name="test" id="form">
                    <label for="name">Name: </label>
                    <input name="name" placeholder="Name" id="name" /><br /><br />
                    <label for="price">Price: </label>
                    <input type="number" step="0.01" name="price" pattern="\d+(\.\d{2})?" placeholder="Example: 124.99" id="price" /><br /><br />
                    <label for="country">Country: </label>
                    <input name="country" placeholder="Country" id="country" /><br /><br />
                    <label for="bearingNumber">Bearing Number:</label>
                    <input type="number" name="bearingNumber" pattern="^[ 0-12]+$" placeholder="Bearing Number" id="bearingNumber" /><br /><br />
                    <label>Type: </label>
                    <input type="radio" name="type" value="INERTIAL"/>Inertial
                    <input type="radio" name="type" value="FIXEDSPOOL"/>Fixed-spool
                    <input type="radio" name="type" value="MULTIPLYING"/>Multiplying
                    <br /><br />
